...
class DeltaConfigSuite extends FunSuite {
  ...
  test("mergeGlobalConfigs") {
    ...
    hadoopConf.set("foo", "foo_val")
    hadoopConf.set("hello", "world")
    val metadataConf = Map("hello"-> "no one", "purple" -> "avocado")
    val mergedConf = DeltaConfigs.mergeGlobalConfigs(hadoopConf, metadataConf)
    assert(mergedConf.contains("foo") && mergedConf.get("foo") == "foo_val")
    assert(mergedConf.contains("hello") && mergedConf.get("hello") == "no one")
    assert(mergedConf.contains("purple") && mergedConf.get("purple") == "avocado")
  }
