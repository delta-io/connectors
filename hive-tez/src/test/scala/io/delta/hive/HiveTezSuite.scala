...
class HiveTezSuite extends HiveConnectorTest {
  ...
  override def createCluster(
    ...
    private val tez = {
      ...
      conf.setInt(YarnConfiguration.YARN_MINICLUSTER_NM_PMEM_MB, 768)
      conf.setInt(YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_MB, 384)
      conf.setInt(YarnConfiguration.RM_SCHEDULER_MAXIMUM_ALLOCATION_MB, 768)
      // Overrides values from the hive/tez-site.
      conf.setInt("hive.tez.container.size", 384)
      conf.setInt(TezConfiguration.TEZ_AM_RESOURCE_MEMORY_MB, 384)
      conf.setInt(TezConfiguration.TEZ_TASK_RESOURCE_MEMORY_MB, 384)
